(function(parent) {
			...
			if (Array.isArray(keys)) {
					...
					return function(elem, idx) {
						return condition(
							keys.map(function(key) {
								return extractValue(elem, key, idx);
							}),
							idx
						);
					};
				...
				return function(elem, idx) {
					return keys.every(function(key) {
						return extractValue(elem, key, idx) === condition;
					});
				};
			}
	...
	parent.Aggregator = function(data) {
		...
		var mapData = function(mapper) {
			return self._data
				.map(function(elem, i) {
					return extractValue(elem, mapper, i);
				});
		};
		...
		self.flatMap = function(mapper) {
			return new parent.Aggregator(
				mapData(mapper)
					.reduce(function(collector, partial) {
						return collector.append(partial);
		...
		self.count = function() {
			...
			return self._data.reduce(function(sum, elem, i) {
				return sum + (cond(elem, i) ? 1 : 0);
			}, 0);
		};
		...
		self.all = function() {
			...
			return self._data.every(function(elem, i) {
				return cond(elem, i);
			});
		};
		...
		self.append = function(elem) {
			if (elem.constructor === parent.Aggregator) {
				return new parent.Aggregator(
					self._data.concat(elem.toArray())
				);
			}
			
			// works for array and element
			return new parent.Aggregator(
				self._data.concat(elem)
			);
		};
		...
		self.getCommonElements = function() {
			...
			for (var i = startIdx; i < arguments.length; ++i) {
				...
				arguments[i]._data.forEach(function(elem, idx) {
	...
	parent.and = function(lhs, rhs) {
		return function(value) {
			return evalCondition(lhs)(value) && evalCondition(rhs)(value);
		};
	};
