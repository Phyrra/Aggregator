(function(parent) {
	...
	var extractValue = function(elem, extractor) {
		if (typeof extractor === 'function') {
			return extractor(elem);
			...
			return function(elem, idx) {
				...
				for (var i = 0; i < keys.length; ++i) {
					...
					if (typeof cond === 'function') {
						result = cond(getDeepValue(elem, key), idx);
					
					// { 'key': val }
					} else {
						result = (getDeepValue(elem, key) === cond);
					}
